...
import { Organization, User, Mood, Conversation, TherapistBooking, JournalEntry } from "@/models";
import nodemailer from "nodemailer";
import * as XLSX from "xlsx";

// In-memory OTP store for MVP/Prototype
...
export class OrgController {
  ...
  static getVerifiedOrgs = asyncHandler(async (req: AuthedRequest, res: Response) => {
  ...
  static sendOtp = asyncHandler(async (req: AuthedRequest, res: Response) => {
    ...
    // Basic check to ensure it's not a public provider (optional, but good for "domain" check)
    const domain = email.split('@')[1];
    const publicDomains = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com'];
    if (publicDomains.includes(domain.toLowerCase())) {
      return res.status(400).json({ error: "Please use an official organization/college domain email." });
    }
    ...
    console.log(`[MOCK EMAIL] Subject: Your MindGod Org Verification Code`);
    ...
    if (process.env.SMTP_USER && process.env.SMTP_PASS) {
      try {
        await transporter.sendMail({
          from: `"MindGod Verification" <${process.env.SMTP_USER}>`,
          to: email,
          subject: "Your MindGod Organization Verification Code",
          html: `<div style="font-family: sans-serif; max-w: 500px; margin: 0 auto;">
            <h2 style="color: #0f172a;">Verify Your Organization</h2>
            <p>You recently requested to register your organization on MindGod.</p>
  ...
  static verifyOtp = asyncHandler(async (req: AuthedRequest, res: Response) => {
    ...
    // OTP matched
    otpStore.delete(email.toLowerCase());
    res.json({ message: "Email verified successfully", verifiedEmail: email.toLowerCase() });
  });

  static onboarding = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const {
      name,
      type,
      officialEmail,
    ...
    res.json({ message: "Organization submitted for review successfully", org });
  });

  /** GET /org/pending-therapists — Get pending therapists for this organization */
  static pendingTherapists = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const user = await User.findById(req.user!.sub);
    if (!user || !user.orgId) return res.status(403).json({ error: "Not associated with an organization" });
  ...
  static verifyTherapist = asyncHandler(async (req: AuthedRequest, res: Response) => {
    ...
    const therapist = await User.findOne({ _id: id, orgId: user.orgId, role: "therapist" });
    if (!therapist) {
      return res.status(404).json({ error: "Therapist not found or not part of this organization" });
    }

    if (!therapist.therapistProfile) {
      return res.status(400).json({ error: "Therapist profile not initialized" });
    }

    therapist.therapistProfile.verified = verified;
    therapist.therapistProfile.verificationStatus = verified ? "verified" : "rejected";
  ...
  static dashboardStats = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const user = await User.findById(req.user!.sub).lean();
    if (!user || !user.orgId) return res.status(403).json({ error: "Organization access required" });

    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);

    // 1. Get all users in this org
    const orgUsers = await User.find({ orgId: user.orgId, role: "user", deletedAt: null }).select("_id department lastActiveAt").lean();
    const userIds = orgUsers.map(u => u._id);

    // 2. Active users (active in last 30 days)
    const activeUsers = orgUsers.filter(u => u.lastActiveAt && u.lastActiveAt >= thirtyDaysAgo).length;

    // 3. Department aggregations
    // Default fallback to "General" if no department
    const deptsMap = new Map<string, { members: number, moodSum: number, moodCount: number, sessions: number }>();
    orgUsers.forEach(u => {
      const d = u.department || "General";
      if (!deptsMap.has(d)) deptsMap.set(d, { members: 0, moodSum: 0, moodCount: 0, sessions: 0 });
      deptsMap.get(d)!.members += 1;
    });

    // Moods in the org
    const moods = await Mood.find({ userId: { $in: userIds }, createdAt: { $gte: thirtyDaysAgo } }).populate("userId", "department").lean();
    moods.forEach(m => {
    ...
    const bookings = await TherapistBooking.find({ userId: { $in: userIds }, createdAt: { $gte: thirtyDaysAgo } }).populate("userId", "department").lean();
    bookings.forEach(b => {
    ...
    const DEPARTMENTS = Array.from(deptsMap.entries()).map(([name, data]) => ({
      id: `dept_${name}`,
      name,
      members: data.members,
      avgMood: data.moodCount > 0 ? parseFloat((data.moodSum / data.moodCount).toFixed(1)) : 0,
      sessions: data.sessions,
      burnoutRisk: data.moodCount > 0 && (data.moodSum / data.moodCount) < 5.0
    }));
    ...
    moods.forEach(m => {
      ...
      if (!moodTrendMap.has(dateStr)) moodTrendMap.set(dateStr, { sum: 0, count: 0 });
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      MOOD_DATA.push({
        date: d,
        mood: moodTrendMap.has(d) ? parseFloat((moodTrendMap.get(d)!.sum / moodTrendMap.get(d)!.count).toFixed(1)) : null
      });
    }

    // 5. Overall metrics
    const totalMoodCount = moods.length;
    const avgTeamMood = totalMoodCount > 0 ? (moods.reduce((sum, m) => sum + m.score, 0) / totalMoodCount).toFixed(1) : "0.0";
    const engagement = orgUsers.length > 0 ? Math.round((activeUsers / orgUsers.length) * 100) : 0;
  ...
  static listUsers = asyncHandler(async (req: AuthedRequest, res: Response) => {
    ...
    const users = await User.find({ orgId: user.orgId, role: "user", deletedAt: null })
    ...
    res.json({
      users: users.map(u => ({
        ...
        status: u.lastActiveAt && u.lastActiveAt >= new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) ? "Active" : "Inactive"
      }))
    });
  });
  ...
  static requestJoin = asyncHandler(async (req: AuthedRequest, res: Response) => {
    ...
    if (org.verificationStatus !== "verified") {
      return res.status(400).json({ error: "Organization is not yet verified on MindGod" });
    }
    ...
    if (existing) {
      return res.json({ message: "Join request already submitted", status: existing.status });
    }
    ...
    if (isAutoApproved) {
      ...
      await user.save();
      return res.json({
        message: "You have been automatically approved based on your organisation email!",
        status: "approved",
        autoApproved: true
      });
    }
    ...
    res.json({
      message: "Join request submitted. Your organisation admin will review and approve.",
      status: "pending",
      autoApproved: false
    });
  });
  ...
  static listJoinRequests = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const user = await User.findById(req.user!.sub).lean();
    if (!user || !user.orgId) return res.status(403).json({ error: "Organization access required" });
  ...
  static approveJoinRequest = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const adminUser = await User.findById(req.user!.sub).lean();
    if (!adminUser || !adminUser.orgId) return res.status(403).json({ error: "Organization access required" });

    const { userId } = req.params;
    const org = await Organization.findById(adminUser.orgId);
    if (!org) return res.status(404).json({ error: "Organization not found" });

    const reqEntry = org.pendingJoinRequests.find(r => r.userId.toString() === userId);
    if (!reqEntry) return res.status(404).json({ error: "Join request not found" });
    ...
    res.json({ ok: true, message: "User approved and linked to organisation" });
  });

  /** PATCH /org/join-request/:userId/reject — Org admin rejects a user */
  static rejectJoinRequest = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const adminUser = await User.findById(req.user!.sub).lean();
    if (!adminUser || !adminUser.orgId) return res.status(403).json({ error: "Organization access required" });
  ...
  static uploadEmails = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const adminUser = await User.findById(req.user!.sub).lean();
    if (!adminUser || !adminUser.orgId) return res.status(403).json({ error: "Organization access required" });
  ...
  static getUserData = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const adminUser = await User.findById(req.user!.sub).lean();
    if (!adminUser || !adminUser.orgId) return res.status(403).json({ error: "Organization access required" });
    ...
    const targetUser = await User.findOne({
      ...
      deletedAt: null
    }).select("fullName phoneMasked department lastActiveAt createdAt").lean();
    ...
    const recentMoods = moods.filter(m => new Date(m.createdAt) >= thirtyDaysAgo);
    const avgMood =
      recentMoods.length > 0
        ? parseFloat((recentMoods.reduce((s, m) => s + m.score, 0) / recentMoods.length).toFixed(1))
        : null;
  ...
  static listMembers = asyncHandler(async (req: AuthedRequest, res: Response) => {
    const adminUser = await User.findById(req.user!.sub).lean();
    if (!adminUser || !adminUser.orgId) return res.status(403).json({ error: "Organization access required" });
    ...
    const mappedUsers = members.map(m => {
      const jr = org.pendingJoinRequests.find(r => r.userId.toString() === (m._id as any).toString());
      return {
        id: m._id,
        ...
        status: m.lastActiveAt && m.lastActiveAt >= thirtyDaysAgo ? "Active" : "Inactive",
        role: "user"
      };
    });
    ...
    const therapists = await User.find({
      ...
      deletedAt: null
    }).select("fullName email phoneMasked lastActiveAt createdAt").lean();
    ...
    const mappedTherapists = therapists.map((t: any) => ({
      ...
      status: t.lastActiveAt && t.lastActiveAt >= thirtyDaysAgo ? "Active" : "Inactive",
      role: "therapist"
    }));
